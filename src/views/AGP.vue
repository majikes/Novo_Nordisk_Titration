...
export default defineComponent({
  ...
  setup() {
    ...
    watch(
      () => subjectListStore.currentSubject.id,
      (subjid) => {
    ...
    const dateBoundsUTCString = computed(() => {
      ...
      if (
        typeof dateBounds.value.min.toLocaleString === "function" &&
        typeof dateBounds.value.max.toLocaleString === "function"
      ) {
        minMaxObj.min = dateBounds.value.min.toLocaleString("en-US", {
          timeZone: "UTC",
        });
        minMaxObj.max = dateBounds.value.max.toLocaleString("en-US", {
          timeZone: "UTC",
        });
    ...
    function getValidDates() {
      ...
      if (
        ...
        one_week_ago_local.value.setDate(
          one_week_ago_local.value.getDate() - 7
        );
        ...
        api
          .getAuth<SubjectDatesFromAPIType>(req_url, tokenComputed.value)
          .then((firstAGPResponse: SubjectDatesFromAPIType) => {
            ...
            if (typeof firstAGPResponse.startAndEndDates !== "undefined") {
              ...
              console.log(
                "start and end dates:",
                firstAGPResponse.startAndEndDates
              );
              const dateBoundMin = new Date(
                firstAGPResponse.startAndEndDates.min_TS * 1000
              );
              const dateBoundMax = new Date(
                firstAGPResponse.startAndEndDates.max_TS * 1000
              );
    ...
    function getSMBGs() {
      ...
      if (
        selected.value !== "" &&
        subjectListStore.currentSubject.interventionArm === 0
      ) {
        ...
        console.log(`request to ${req_url}`);
        api
          .getAuth<SMBGsAndHyposFromAPIType>(req_url, tokenComputed.value)
          .then((response: SMBGsAndHyposFromAPIType) => {
            console.log(`${endpoint} success!`);
            console.log(response);
