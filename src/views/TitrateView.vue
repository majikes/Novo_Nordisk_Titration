<template>
  ...
  >
    ...
    <div class="flex justify-end content-end">
      <SubjectDropdown
        v-model="selected"
        :titratable-only="true"
        :disabledProp="modalVisible"
      />
    </div>
    ...
    <div v-else-if="titratePageVisibility.visible">
      ...
      <div class="grid grid-cols-12 min-h-[400px] py-1">
        ...
        <div class="grid">
          <div class="pl-3 py-2" id="tir1-caption">
            <span class="font-semibold">Prev week</span>
          </div>
          <TiRChart
      ...
      <div
        class="grid grid-cols-2 justify-between content-end p-4 gap-2 bg-gray-200 rounded-lg my-4"
      >
        <div
        ...
        <div class="flex items-center justify-end">
          ...
          >
            <div class="btn force-center-content w-52">History</div>
          </router-link>
        </div>
        <div
          class="flex justify-between rounded-lg bg-white px-4 py-3 w-full"
        ...
        >
          <div
            class="force-center-content"
            :class="{
              'text-gray-400': !subjectListStore.currentSubjectNewRec,
              'font-semibold': subjectListStore.currentSubjectNewRec,
            }"
          >
            {{ subjectListStore.currentSubjectNewRec ? "NEW" : "Latest" }}
            recommended basal insulin dose:
          </div>
          <div
            class="force-center-content px-2 font-semibold"
        ...
        <div class="grid grid-cols-2">
          <div class="flex">
            <div class="force-center-content px-2 font-semibold">
              <input
                type="text"
                id="small-input"
                class="block w-12 p-2 border rounded-lg text-md focus:ring-blue-500 focus:border-blue-500 bg-gray-700 border-gray-600 placeholder-gray-500 text-white disabled:bg-white disabled:text-gray-400 disabled:border-transparent disabled:placeholder-gray-400"
                :disabled="newRecDisplayDisabled"
                placeholder="N/A"
                v-model="newDoseModel"
              />
            </div>
            <div
              class="flex px-2 items-center gap-2"
          ...
          <div class="flex">
            <button
              class="btn force-center-content w-52"
              :disabled="confirmAndSendDisabled"
    ...
    <div v-if="debugModeStore.debugMode">
      ...
      <div>
        subjectListStore.subjectListTitratable:
        {{ subjectListStore.subjectListTitratable }}
      </div>
      <div>
        subjectListStore.subjectListSorted:
        {{ subjectListStore.subjectListSorted }}
      </div>
    </div>
  </div>
...
function graphData() {
  ...
  const startTS = Math.floor(
    new Date(one_week_ago_local.value).valueOf() / 1000
  );
  console.log(
    `graphing from ${one_week_ago_local.value} to ${yesterday_local.value}...`
  );
  ...
  const req_url = `${apiRootURL}/agp?subject_id=${
    selected.value
  }&day1=${startTS.valueOf()}&day2=${endTS.valueOf()}`;
  console.log(`request to ${req_url}`);
  api
    .getAuth<SubjectGraphable>(req_url, tokenComputed.value)
    .then((subjectGraphable: SubjectGraphable) => {
      console.log("success!");
...
const newDoseIsDigits = computed(() => {
  return (
    typeof newDoseModel.value === "string" &&
...
const newDoseProblems = computed(() => {
  let problemStr = "";
  if (
    !isEmpty(subjectListStore.currentSubject.dose_problems) ||
    !isEmpty(subjectListStore.currentSubject.rec_dose_problems)
  ) {
...
function submitTitration() {
  ...
  const inputJSON = JSON.stringify({
    ...
    requestor: {
      ...
      role: groupComputed.value.includes("physician")
        ? "physician"
        : `${groupComputed.value}`,
    },
  });
  ...
  const requestObj = {
    ...
    stateMachineArn:
      "arn:aws:states:us-east-1:582050746740:stateMachine:Novo_Titration_Optimize_Save_and_Notify_Recommendations",
  };
  ...
  api
    ...
    .then((response: any) => {
      ...
      if (typeof response["startDate"] !== undefined) {
        ...
        errors.errorLog(
          `${componentName}; request to ${req_url}: ${tmpErr}`,
          true
        );
      }
    })
...
const confirmAndSendDisabled = computed(() => {
  return !newDoseValid.value || newRecShowModalDisabled.value;
});
