<template>
  <div class="add-participant">
    ...
    <div class="grid grid-cols-2 mt-4">
      <div class="col-span-2" v-if="debugModeStore.debugMode">
        <div class="w-64" @click="fakefill">fakefill</div>
      </div>
      <!-- form fields -->
      <div class="adduser-form-cell">
          ...
          <div class="force-center-content">
            ...
            <ToolTip displaySide="mid">
              <div class="text-sm px-2">
                ...
                <div v-for="subjid in subjectIDs" :key="subjid">
                  {{ subjid }}
                </div>
              </div>
            </ToolTip>
        ...
        <input
          type="text"
          :disabled="modalVisible || subjectsLoading"
          v-model="inputObj.id"
          class="custom-text-input peer form-control"
          :class="{ 'custom-valid': idValid, 'custom-invalid': !idValid }"
          id="validationCustom01"
          :placeholder="idFieldLoading"
          required
          pattern="\d{6}"
        />
        <div
      ...
      <div class="adduser-form-cell">
        ...
        <input
          ...
          class="adduser-text-input peer"
          id="validationCustom10"
          placeholder="fake.name@example.com"
          required
        />
      ...
      <div
        v-if="groupComputed.includes('crc') || groupComputed.includes('admin')"
        class="adduser-form-cell"
      >
        <!-- <label for="validationCustom02"></label> -->
        <label for="validationCustom13">Physician</label>
        <select
          :disabled="modalVisible || physAndCRCLoading"
          v-model="inputObj.physician"
          class="adduser-select-input peer"
          id="validationCustom02"
          required
        >
          <option disabled value="" selected>
            {{ physAndCRCLoading ? "Loading..." : "Select Physician..." }}
          </option>
          <option
      ...
      <div
        v-if="
          groupComputed.includes('physician') || groupComputed.includes('admin')
        "
        class="adduser-form-cell"
      >
        <!-- <label for="validationCustom02"></label> -->
        ...
        <select
          :disabled="modalVisible || physAndCRCLoading"
          v-model="inputObj.crc"
          class="adduser-select-input peer"
          id="validationCustom02"
          required
        >
          <option disabled value="" selected>
            {{ physAndCRCLoading ? "Loading..." : "Select CRC..." }}
          </option>
          <option v-for="crc in crcs" :value="crc[0]" :key="crc[0]">
            {{ crc[1] }} {{ crc[2] }}
          </option>
        </select>
      ...
      <div class="adduser-form-cell">
        ...
        <div class="invalid-feedback">
          Weight must be a positive number from {{ weightMin }} to
          {{ weightMax }}.
        </div>
      </div>
      ...
      <div class="adduser-form-cell">
        ...
        />
        <div class="invalid-feedback">
          Height must be a positive integer from {{ heightMin }} to
          {{ heightMax }}.
        </div>
      </div>
      ...
      <div class="adduser-form-cell">
        ...
        <select
          :disabled="modalVisible"
          v-model="inputObj.sex"
          class="adduser-select-input peer"
          id="validationCustom02"
          required
        >
          <option disabled value="" selected>Select sex...</option>
          <option v-for="sex in sexes" :value="sex" :key="sex">
            {{ sex }}
          </option>
        </select>
      ...
      <div class="adduser-form-cell">
        ...
        <input
          :disabled="modalVisible"
          type="number"
          v-model="inputObj.basalDose"
          class="adduser-text-input peer"
          id="validationCustom04"
          placeholder="Basal Dose"
          required
          :step="basalStep"
          :min="basalMin"
          :max="basalMax"
        />
        <div class="invalid-feedback">
          Basal Dose must be a numeric value from {{ basalMin }} to
          {{ basalMax }}.
        </div>
      </div>
      ...
      <div class="adduser-form-cell">
        ...
        <input
          :disabled="modalVisible"
          type="time"
          v-model="inputObj.basalDoseTime"
          class="adduser-text-input peer"
          id="validationCustom04"
          placeholder="Basal Dose Time"
          required
          min="00:00"
          max="23:59"
        />
      ...
      <div class="adduser-form-cell">
        ...
        <select
          :disabled="modalVisible"
          v-model="inputObj.interventionArm"
          class="adduser-select-input peer"
          id="validationCustom03"
          required
        >
          ...
          <option v-for="arm in studyArms" :value="arm" :key="arm">
            {{ (armMap as any)[arm] }}
          </option>
        </select>
      ...
      <div class="adduser-form-cell">
        ...
        <input
          :disabled="modalVisible"
          type="number"
          v-model="inputObj.reservoirSize"
          class="adduser-text-input peer"
          id="validationCustom04"
          placeholder="Reservoir Size"
          required
          :step="reservoirStep"
          :min="reservoirMin"
          :max="reservoirMax"
        />
        <div class="invalid-feedback">
          Reservoir size must be a numeric value from {{ reservoirMin }} to
          {{ reservoirMax }}.
        </div>
      </div>
      ...
      <div class="adduser-form-cell">
        ...
        />
        <div class="invalid-feedback">
          Max dose must be a numeric value from {{ maxDoseMin }} to
          {{ maxDoseMax }}.
        </div>
      </div>
      ...
      <div class="adduser-form-cell">
        ...
        />
        <div class="invalid-feedback">
          Min dose must be a numeric value from {{ minDoseMin }} to
          {{ minDoseMax }}.
        </div>
      </div>
      ...
      <div class="adduser-form-cell">
        ...
        <input
          :disabled="modalVisible"
          type="number"
          v-model="inputObj.doseIncrement"
          class="adduser-text-input peer"
          id="validationCustom04"
          placeholder="Dose Increment"
          required
          :step="doseIncrementStep"
          :min="doseIncrementMin"
          :max="doseIncrementMax"
        />
        <div class="invalid-feedback">
          Dose Increment must be a numeric value from {{ doseIncrementMin }} to
          {{ doseIncrementMax }}.
        </div>
      </div>
    </div>
    ...
    <div v-if="debugModeStore.debugMode">
      ...
      <div>inputObjSubmittable: {{ inputObjSubmittable }}</div>
    </div>
    <!-- "sumbit" control -->
    <div class="adduser-form-cell flex justify-center">
      <div class="adduser-form-cell flex justify-center">
        <button
          class="btn w-96"
          :disabled="!inputValid.valid && !debugModeStore.debugMode"
          @click="showModal"
        >
    ...
    <div
      v-if="modalVisible"
      class="adduser-modal bg-transparent"
      id="modal-container"
    >
      ...
      >
        ...
        <div class="p-4 rounded-lg bg-white">
          ...
          <div class="p-4 mb-4 rounded-lg bg-gray-100">
            <div
              v-for="(value, key) in inputObjDisplayed"
              class="grid grid-cols-2 px-4 py-1"
              :key="key"
            >
        ...
        <div class="p-4 rounded-lg border-2 border-orange-300">
          <div class="control-row font-semibold">
            Confirm information and add subject to system?
          </div>
          <div class="flex justify-between gap-4 mt-4 mb-2">
            <button
              class="profile-confirm-btn w-64 bg-gray-100"
              @click="hideModal"
            ...
            <button
              class="profile-confirm-btn w-64 bg-emerald-100 font-semibold"
              @click="addUser"
              :disabled="addUserLoading"
            >
            ...
            <button
              ...
              class="profile-confirm-btn w-64 bg-rose-300 font-semibold"
...
export default defineComponent({
  ...
  setup() {
    ...
    const physDict = computed(() => {
      const retDict = {} as any;
      for (const phys of physicians.value) {
        retDict[phys[0]] = `${phys[1]} ${phys[2]}`;
      }
      return retDict;
    });
    const crcDict = computed(() => {
      const retDict = {} as any;
      for (const crc of crcs.value) {
        retDict[crc[0]] = `${crc[1]} ${crc[2]}`;
      }
      return retDict;
    });
    ...
    const inputObjSubmittable = computed(() => {
      const inputClone = cloneDeep(inputObj.value);
      if (groupComputed.value.includes("physician")) {
        inputClone.physician = auth.user?.username;
        inputClone.otherSupervisorId = inputClone.crc;
      } else if (groupComputed.value.includes("crc")) {
        inputClone.crc = auth.user?.username;
        inputClone.otherSupervisorId = inputClone.physician;
      }
      return inputClone;
    });

    const inputObjDisplayed = computed(() => {
      const inputClone = cloneDeep(inputObj.value);
      if (groupComputed.value.includes("physician")) {
        inputClone.physician = auth.user?.username;
        inputClone.crc = `${crcDict.value[inputObj.value.crc]} (ID: ${
          inputObj.value.crc
        })`;
      } else if (groupComputed.value.includes("crc")) {
        inputClone.crc = auth.user?.username;
        inputClone.physician = `${
          physDict.value[inputObj.value.physician]
        } (ID: ${inputObj.value.physician})`;
      }
      inputClone.interventionArm = (armMap as any)[inputObj.value.interventionArm]
      return inputClone;
    });
    ...
    const idRegex = /\d{6}/;
    const idIsDigits = computed(() => {
      return inputObj.value.id.match(idRegex) !== null;
    });
    const idIsLen = computed(() => {
      return inputObj.value.id.length === 6;
    });
    const idValid = computed(() => {
      return idNotTaken.value && idIsDigits.value && idIsLen.value;
    });
    ...
    const inputValid = computed(() => {
      ...
      if (!idIsDigits.value) {
        problems.push("ID must be comprised of only digits");
      }
      if (!idIsLen.value) {
        problems.push("ID must be 5 characters in length");
      }
      if (!idNotTaken.value) {
        problems.push("ID must not already be assigned to an existing subject");
      }
      ...
      if (
        ...
        if (!physicianValid) {
          problems.push("Select a physician");
        }
      ...
      const weightInRange =
        Number(inputObj.value.weight) >= weightMin &&
        Number(inputObj.value.weight) <= weightMax;
      if (!weightIsNum) {
        problems.push("Input a weight value");
      }
      if (!weightInRange) {
        problems.push(
          `Weight input of ${inputObj.value.weight} is not between ${weightMin} and ${weightMax}`
        );
      }
      ...
      const heightInRange =
        Number(inputObj.value.height) >= heightMin &&
        Number(inputObj.value.height) <= heightMax;
      if (!heightIsNum) {
        problems.push("Input a height value");
      }
      if (!heightInRange) {
        problems.push(
          `Height input of ${inputObj.value.height} is not between ${heightMin} and ${heightMax}`
        );
      }
      ...
      const ageInRange =
        Number(inputObj.value.age) >= ageMin &&
        Number(inputObj.value.age) <= ageMax;
      if (!ageIsNum) {
        problems.push("Input an age value");
      }
      if (!ageInRange) {
        problems.push(
          `Age input of ${inputObj.value.age} is not between ${ageMin} and ${ageMax}`
        );
      }
      ...
      const basalInRange =
        Number(inputObj.value.basalDose) >= basalMin &&
        Number(inputObj.value.basalDose) <= basalMax;
      if (!basalIsNum) {
        problems.push("Input a basal dose value");
      }
      if (!basalInRange) {
        problems.push(
          `Basal dose input of ${inputObj.value.basalDose} is not between ${basalMin} and ${basalMax}`
        );
      }
      ...
      const reservoirInRange =
        Number(inputObj.value.reservoirSize) >= reservoirMin &&
        Number(inputObj.value.reservoirSize) <= reservoirMax;
      if (!reservoirIsNum) {
        problems.push("Input a reservoir size value");
      }
      if (!reservoirInRange) {
        problems.push(
          `Reservoir size input of ${inputObj.value.reservoirSize} is not between ${reservoirMin} and ${reservoirMax}`
        );
      }
      ...
      const maxDoseInRange =
        Number(inputObj.value.maxDose) >= maxDoseMin &&
        Number(inputObj.value.maxDose) <= maxDoseMax;
      if (!maxDoseIsNum) {
        problems.push("Input a max dose value");
      }
      if (!maxDoseInRange) {
        problems.push(
          `Max dose input of ${inputObj.value.maxDose} is not between ${maxDoseMin} and ${maxDoseMax}`
        );
      }
      ...
      const minDoseInRange =
        Number(inputObj.value.minDose) >= minDoseMin &&
        Number(inputObj.value.minDose) <= minDoseMax;
      if (!minDoseIsNum) {
        problems.push("Input a min dose value");
      }
      if (!minDoseInRange) {
        problems.push(
          `Min dose input of ${inputObj.value.minDose} is not between ${minDoseMin} and ${minDoseMax}`
        );
      }
      ...
      const doseIncrementIsNum =
        typeof inputObj.value.doseIncrement === "number";
      const doseIncrementInRange =
        Number(inputObj.value.doseIncrement) >= doseIncrementMin &&
        Number(inputObj.value.doseIncrement) <= doseIncrementMax;
      if (!doseIncrementIsNum) {
        problems.push("Input a dose increment value");
      }
      if (!doseIncrementInRange) {
        problems.push(
          `Dose increment input of ${inputObj.value.doseIncrement} is not between ${doseIncrementMin} and ${doseIncrementMax}`
        );
      }
    ...
    function addUser() {
      ...
      const requestObj = {
        ...
        new_user: {
          ...
          otherSupervisorId: inputObjSubmittable.value.otherSupervisorId,
          role: "participant",
          email: inputObj.value.email,
          weight: inputObj.value.weight,
          height: inputObj.value.height,
    ...
    function fakefill() {
      inputObj.value.id = "411999";
      // inputObj.value.crc =
      // inputObj.value.physician =
      // inputObj.value.otherSupervisorId =
      inputObj.value.email = "fake.name@email.web";
      inputObj.value.weight = 123;
      inputObj.value.height = 123;
      inputObj.value.age = 12;
      inputObj.value.sex = "Male";
      inputObj.value.basalDose = 50;
      inputObj.value.basalDoseTime = "10:49";
      inputObj.value.startDate = "2024-04-06";
      inputObj.value.timezone = "America/New_York";
      inputObj.value.interventionArm = "0";
      inputObj.value.modelNumber = "asdf";
      inputObj.value.serialNumber = "asdf";
      inputObj.value.reservoirSize = 500;
      inputObj.value.maxDose = 50;
      inputObj.value.minDose = 50;
      inputObj.value.doseIncrement = 2;
    }
