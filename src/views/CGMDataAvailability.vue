...
//   return retArr;
// });
const cgmAvailabilityPercentagesValid = ref([] as CGMDataAvailFrontendType[]);

const filterByEnrolled = ref(true);

const cgmAvailabilityPercentagesValidAndEnrolled = computed(() => {
  return [...cgmAvailabilityPercentagesValid.value].filter((participant) => {
...
const cgmAvailabilityPercentagesValidByDate = computed(() => {
  return [...cgmAvailabilityPercentagesValidAndEnrolled.value].sort((a, b) => {
...
function loadSubjectCGMAvail(
  ...
  if (index !== -1) {
    ...
    if (participantCGMAvail.empty || force) {
      ...
      if (
        typeof selectedTimezone.value !== "undefined" &&
        selectedTimezone.value !== "undefined"
      ) {
      ...
      api
        ...
        .then((response: CGMDataFromAPIType) => {
          console.log(
            `successful ${endpoint} request for participant ${participantCGMAvail.username}`
          );
