<template>
  <div
    v-if="auth.authStatus === 'authenticated' && auth.route === 'authenticated'"
    id="content-wrapper"
  ...
  >
    ...
    <nav class="flex flex-row justify-center my-4">
      ...
      >
        <div class="flex">
          <!-- https://feathericons.dev/?search=alert-triangle&iconset=feather -->
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            width="24"
      ...
      <router-link
        v-if="
          groupComputed.includes('physician') || groupComputed.includes('crc')
        "
        class="nav-link group"
      ...
      >
        <div class="flex">
          ...
          {{
            groupComputed.includes("admin") ? "Physician / CRC" : "Admin"
          }}
          Management
        </div>
      </router-link>
      ...
      <router-link
        v-if="!groupComputed.includes('participant')"
        class="nav-link group"
        :to="{ name: 'AppDownload' }"
      >
        <div class="flex">
    ...
    <!-- main view -->
    <router-view
      class="mx-auto max-w-[900px] overflow-y-scroll overflow-x-visible flex-1 main-container"
    />
    <!-- footer / drawer -->
