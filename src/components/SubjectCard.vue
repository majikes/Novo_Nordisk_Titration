<template>
  ...
  >
    <div class="mb-2 basis-full" id="header">
      <div>{{ card.subject_id }}</div>
    </div>
    <!-- TiR bar and tooltip -->
    <!-- <div
      class="w-8 mb-3 rounded-sm relative"
      :class="{ 'ring-2 ring-gray-300': tirInvalid }"
    >
      <TiRChartAlt
        class="h-48 peer"
        :loaded="loaded"
        :tirData="card.tirData"
        :tirheight="192"
        :tirwidth="32"
        :radius="2"
        :gap="1"
    ...
    <!-- <div class="grid content-center place-items-center" id="circle-container">
      <CircleDataQualityChart
        class="w-36 mx-auto peer"
        :loaded="loaded"
        :dataQualityBreakdown="card.dataQualityBreakdown"
        :tConnectStatus="card.tconnectStatus"
      />
