<template>
  <div class="tir-chart" ref="tirContainer">
    <svg class="svg-demo" :viewBox="viewBox">
      <g class="series" >
        <rect v-for="(d, i) in visibleBarData" 
          :key="i" 
          :x="xScale('TiR')" 
          :y="yScale(startHeights[i])" 
          :ry="radius"
          :height="yScale(d - barGap)" 
...
export default defineComponent({
  ...
  props: {
    tirData: {
      ...
      type: Object as PropType<TiRType>
    },
    loaded: {
      required: true,
  ...
  setup(props) {
    ...
    return {
      barGap, data, tirContainer, viewBox, width, height, startHeights,
      xScale, yScale, colorScale, visibleBarData
    }
  }
})
</script>
